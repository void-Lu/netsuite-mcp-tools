...
| 状态 | 文案 | 点击行为 |
| --- | --- | --- |
| 未配置（无有效 clientId） | `NetSuite MCP：未配置` | 打开 `environment.json` + 弹出操作下拉框 |
| 未配置（有有效 clientId） | `NetSuite MCP：未配置` | 弹出操作下拉框 |
| 只读已连接 | `NetSuite MCP：只读已连接` | 弹出操作下拉框 |
| 写入已启用 | `NetSuite MCP：写入已启用` | 弹出操作下拉框 |
| 需要修复 | `NetSuite MCP：需要修复` | 弹出端口占用错误提示 |

VS Code 重启后，即使 `environment.json` 中 profile 标记为 `verified`，若内存中无 OAuth token，状态栏仍显示"未配置"而非误报"已连接"。
...
4. 点击状态栏的 `NetSuite MCP：未配置` 按钮。

## 首次配置

1. 点击状态栏的 `NetSuite MCP：未配置` 按钮。扩展会创建并打开 `/.netsuite-mcp/environment.json`，同时弹出操作下拉框；之后所有人工配置都在这个文件完成，不会逐字段弹窗询问。
2. 在模板中编辑以下人工配置：
...
4. 回到 `environment.json` 填写 `clientId` 并保存，然后点击状态栏按钮，从下拉框选择 **测试连接**。扩展会启动仅限 `127.0.0.1` 的临时回调、打开系统浏览器完成 NetSuite 登录/同意，再执行 MCP `initialize` 与 `tools/list` 健康检查，不读取业务数据；健康检查成功后会把档案标记为已验证，并将该工作区固定端口加入共享的 `allocatedPorts` 和当前主机的插件用户级排除清单。
5. 验证后，点击状态栏按钮，从下拉框选择 **生成 Agent 配置**。它会在用户确认后写入：
...
- 在只读已连接状态下，点击状态栏并选择 **启用写入连接**。若当前 environment 缺少 write profile，扩展会自动创建 draft 模板；若 clientId 为空，会弹出输入框引导填写。
- 选择 write profile 后，若该 profile 尚未验证或当前会话无内存 token，扩展会先打开浏览器完成 OAuth 授权和健康检查，再启用写入端点。
- 写入端点在 VS Code 重启/关闭时自动关闭；NetSuite Role 才是最终权限边界。
- 代理不会通过工具名称猜测读写权限，也不会重放网络超时、5xx 或工具业务错误。

## 重新授权与清理

- 授权态只在当前 VS Code 会话保留；重启或停用扩展后，点击状态栏并从下拉框选择 **重新授权** 即可重新在浏览器中完成授权。
- 扩展不会自动撤销 NetSuite 远端 Integration 或其已授予的访问权限。
- **清理 MCP 配置**操作会扫描 `.vscode/mcp.json`、`.mcp.json` 和 `~/.codex/config.toml`，删除所有由本扩展托管（名称以 `netsuite-mcp-` 开头）的 server 条目，保留无关配置。若需要移除连接，请手动编辑 `environment.json` 删除对应 profile，再执行清理。
...
| 本机代理提示需要授权 | 点击状态栏，从下拉框选择 **重新授权**，在系统浏览器中完成当前会话的授权。 |
